window.MENU_DATA = {
  morning: {
    sections: [
      {
        
        timing: "Available from Fajr till Zuhr / متاح من الفجر حتى الظهر",
        note: "(Includes 1 Roti) / (يشمل 1 روتي)",
        items: [
          { name: "Paya", price: "6", calories: "٣٢٠", arabic: "بايا" },
          { name: "Keema", price: "4", calories: "٣٤٥", arabic: "خيـمة" },
          { name: "Bheja", price: "6", calories: "٢٧٠", arabic: "بيـجا" },
          { name: "Gurda", price: "6", calories: "٢٠٠", arabic: "جوردا" },
          { name: "Alu Palak", price: "3", calories: "١٨٠", arabic: "ألو بالسبانخ" },
          { name: "Alu Paratha", price: "3", calories: "٢٢٠", arabic: "ألو باراثا" },
          { name: "Omlet (Double Egg)", price: "6", calories: "١٨٠", arabic: "بيض" },
          { name: "Roti", price: "1", calories: "١٢٠", arabic: "روتي" }
        ]
      }
    ]
  },
  maincourse: {
    sections: [
      {
        timing: "Available from Zuhr till 11.30pm / متاح من الظهر حتى 11:30 مساءً",
        note: "(Includes Rice & 2 Rotis & Dal) / يشمل الأرز و 2 روتي و عدس",
        items: [
          { name: "Mutton Curry", price: "12", calories: "٣٩٨", arabic: "موتون كاري" },
          { name: "Chana Masala", price: "12", calories: "٣٢٦", arabic: "شانا ماسالا" },
          { name: "Kadhai Gosht", price: "12", calories: "٣٦٠", arabic: "قدر لحم" },
          { name: "Palak Gosht", price: "13", calories: "٣٢٠", arabic: "شانا لحم" },
          { name: "Chana Gosht", price: "13", calories: "٣٣٠", arabic: "شانا لحم" },
          { name: "Alu Gosht", price: "12", calories: "٣١٠", arabic: "لحم بالبطاطس" },
          { name: "Chicken Masala", price: "12", calories: "٢٩٠", arabic: "دجاج ماسالا" },
          { name: "Kadhai Chicken", price: "12", calories: "٣١٠", arabic: "دجاج بالقدر" },
          { name: "Alu Matar", price: "12", calories: "٢٦٠", arabic: "بطاطس وبازلاء" },
          { name: "Alu Palak Matar", price: "12", calories: "٢٥٠", arabic: "بطاطس سبانخ وبازلاء" },
          { name: "Anda Masala", price: "12", calories: "٣٠٠", arabic: "بيض ماسالا" },
          { name: "Anda Ghotala", price: "13", calories: "٣٥٠", arabic: "بيض غوتالا" },
          { name: "Keema Ghotala", price: "13", calories: "٣٨٠", arabic: "قيمة غوتالا" },
          { name: "Keema Fry", price: "12", calories: "٣٧٠", arabic: "قيمة مقلية" },
          { name: "Alu Keema", price: "12", calories: "٣٣٠", arabic: "قيمة بالبطاطس" },
          { name: "Dal Gosht", price: "13", calories: "٢٨٠", arabic: "لحم بعدس" },
          { name: "Paya Fry", price: "12", calories: "٣٠٠", arabic: "بايا مقلية" },
          { name: "Bheja Fry", price: "12", calories: "٣٥٠", arabic: "دماغ مقلية" },
          { name: "Dal Fry", price: "12", calories: "٢٧٠", arabic: "عدس مقلي" },
          { name: "Dal", price: "5", calories: "١٠٠", arabic: "عدس إضافي" },
          { name: "Roti", price: "1", calories: "١٢٠", arabic: "روتي إضافي" }
        ]
      }
    ]
  },
  evening: {
    sections: [
      {
        timing: "Available from 4:00 PM till 11:00 PM / متاح من الساعة 4:00 مساءً حتى 11:00 مساءً",
        items: [
          { name: "Shami Kabab", price: "4", calories: "٤١٥", arabic: "كباب شامي" },
          { name: "Samosa (3 Pieces)", price: "2", calories: "٢٦٠", arabic: "سمبوسة (3 قطع)" },
          { name: "Bhajiya (5 Pieces)", price: "2", calories: "٢٠٠", arabic: "باجيا (5 قطع)" },
          { name: "Namakpara (1 Plate)", price: "2", calories: "١٨٠", arabic: "نمك بارا (طبق)" },
          { name: "Jalebi (1 Plate)", price: "2", calories: "٢٢٠", arabic: "جليبي (طبق)" },
          { name: "Namakpara (1 kg)", price: "30", calories: "١٨٠٠", arabic: "نمك بارا (1 كجم)" },
          { name: "Jalebi (1 kg)", price: "30", calories: "٢١٠٠", arabic: "جليبي (1 كجم)" }
        ]
      }
    ]
  },
  fullday: {
    sections: [
      {
        items: [
          { name: "Milk Tea", price: "1", calories: "١٠", arabic: "شاي بالحليب" },
          { name: "Black Tea", price: "1", calories: "٥", arabic: "شاي أسود" },
          { name: "Coffee", price: "2", calories: "١٠", arabic: "قهوة" }
        ]
      }
    ]
  },
  fridayspecial: {
    sections: [
      {
        items: [
          { name: "Mutton Biryani (1 Plate)", price: "15", calories: "٤٨٠", arabic: "برياني لحم (طبق)" }
        ]
      }
    ]
  }
};
